	...
	, deviceImpl(nullptr)
{
	...
	connect(fw, &QFutureWatcher<bool>::started, ui->btnVerify, &AnimationPushButton::startAnimation,
		Qt::QueuedConnection);
	...
	connect(fwScan, &QFutureWatcher<int>::started, ui->btnScan, &AnimationPushButton::startAnimation,
		Qt::QueuedConnection);
	...
	connect(ui->comboBoxSerialPort, &QComboBox::textActivated, this, [=]() { Q_EMIT uriChanged(getSerialPath()); });
	connect(ui->comboBoxBaudRate, &QComboBox::textActivated, this, [=]() { Q_EMIT uriChanged(getSerialPath()); });
	...
	connect(ui->editUri, &QLineEdit::returnPressed, this, [=]() { Q_EMIT ui->btnVerify->clicked(); });
	connect(ui->editUri, &QLineEdit::textChanged, this,
		[=](QString uri) { ui->btnVerify->setEnabled(!uri.isEmpty()); });
	connect(
		this, &ScopyHomeAddPage::deviceInfoAvailable, this,
		[=](QMap<QString, QString> ctxInfo) {
