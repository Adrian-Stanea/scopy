	...
	, registerMapTemplate(registerMapTemplate)
{
	...
	if(registerMapTemplate) {
		...
		QObject::connect(
			registerMapTableWidget, &gui::RegisterMapTable::registerSelected, this,
			[=](uint32_t address) { registerChanged(registerMapTemplate->getRegisterTemplate(address)); });
		...
		QObject::connect(registerController, &gui::RegisterController::registerAddressChanged, this,
				 [=](uint32_t address) {
