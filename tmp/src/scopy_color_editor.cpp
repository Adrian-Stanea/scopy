	...
	, m_sh(nullptr)
{
	...
	connect(m_ui->copyBtn, &QPushButton::clicked, this, &ScopyColorEditor::copy);
	connect(m_ui->removeBtn, &QPushButton::clicked, this, &ScopyColorEditor::remove);
	connect(m_ui->stylesheetsCmbBox, &QComboBox::currentTextChanged, this, &ScopyColorEditor::stylesheetSelected);
	connect(m_ui->searchLineEdit, &QLineEdit::textChanged, this, &ScopyColorEditor::search);
	connect(m_ui->tabWidget, &QTabWidget::currentChanged, this, &ScopyColorEditor::tabChanged);
	connect(m_ui->nextMatchBtn, &QPushButton::clicked, this, &ScopyColorEditor::nextMatch);
	connect(m_ui->prevMatchBtn, &QPushButton::clicked, this, &ScopyColorEditor::prevMatch);
	connect(m_ui->saveBtn, &QPushButton::clicked, this, &ScopyColorEditor::advancedEditorChanged);
	connect(m_ui->advancedEditorTextEdit, &QTextEdit::textChanged, [=]() { m_ui->saveBtn->setEnabled(true); });
	connect(m_ui->restartBtn, &QPushButton::clicked, [=]() { adiscope::ApplicationRestarter::triggerRestart(); });
...
{
	m_ui->advancedEditorTextEdit->find(m_ui->searchLineEdit->text(),
